...
@pytest.mark.xfail(
    strict=True,
    reason="B-018 deferred: criterion 3 correlates motion_delta against CUMULATIVE "
    "global motion (corr~0.06 for pure whole-body motion), so the artifact filter "
    "never fires; results-changing fix (delta->abs) awaiting sign-off",
)
def test_b018_artifact_filter_discards_pure_whole_body_motion():
    """filter_episodes_artifacts discards a pure whole-body-motion episode (identical cumulative sinusoid on every patch) at default max_global_corr. Regression for B-018 (deferred)."""
    T = 40
