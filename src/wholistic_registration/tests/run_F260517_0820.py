...
Output (under f260517_0820/):
  ...
  ref_surface_mem/         reference membrane sampled directly at fixed z planes
  ref_surface_sparseCell/  reference sparse-cell sampled directly at fixed z planes
  diagnostics/             loader-compatible diagnostics and phase_new files
    phase_new_f%06d.npy    raw phase_new, one .npy per frame
    ref_shape.npy          cropped reference shape (Z, Y, X)
    *.csv, *.json, *.npy   metrics and run metadata
"""
...
DIRS = {
    ...
    "phase_new":              DIAGNOSTICS_DIR,
}
for d in {DIAGNOSTICS_DIR, *DIRS.values()}:
...
np.save(str(BASE_OUT / "z_init.npy"), z_init.astype(np.float32), allow_pickle=False)
np.save(str(BASE_OUT / "fixed_target_z.npy"), fixed_target_z.astype(np.float32), allow_pickle=False)
np.save(
    str(DIAGNOSTICS_DIR / "ref_shape.npy"),
    np.asarray(ref_mem_raw.shape, dtype=np.int64),
    allow_pickle=False,
)
with open(DIAGNOSTICS_DIR / "projection_params.json", "w", encoding="utf-8") as f:
    json.dump({
        "moving_source_path": F260517_mov_path,
        "reference_source_path": F260517_ref_path,
        "reference_crop_z": [90, 310],
        "reference_channels": {"sparseCell": 0, "mem": 1},
        "K": K,
        "T": T,
        "surface_upsample_factor": 2,
        "projection_z_window": Z_WINDOW,
        "projection_fill_value": FILL_VALUE,
        "projection_downsample_xy": 1,
        "projection_xy_splat_mode": "subpixel_footprint",
        # Names consumed by dashboard/native/app/run.cpp.
        "reference_crop": {
            "z0": 90,
            "z1": 310,
            "channel_mem": 1,
        ...
        "phase_saved": "phase_new_before_xy_upsampling",
        "phase_filename_pattern": "phase_new_f%06d.npy",
    }, f, indent=2)


# ---------------------------------------------------------------------------
# Reference surfaces and phase verification
# ---------------------------------------------------------------------------
_ref_surface_saved = False


def sample_reference_at_fixed_z(ref_volume, target_z_planes, ref_volume_order):
    """Directly sample a reference volume on complete fixed-z XY planes.

    Unlike moving-data projection, this does not use phase coordinates,
    z-window gating, or forward splatting. Non-integer target z values are
    linearly interpolated between adjacent reference z slices.
    """
    volume = np.asarray(ref_volume, dtype=np.float32)
    if ref_volume_order == "xyz":
        volume = volume.transpose(2, 1, 0)
    elif ref_volume_order != "zyx":
        raise ValueError(f"Unsupported reference volume order: {ref_volume_order}")

    z_count = volume.shape[0]
    target_z = np.asarray(target_z_planes, dtype=np.float32)
    if np.any(target_z < 0) or np.any(target_z > z_count - 1):
        raise ValueError(
            f"Target z outside reference volume: min={target_z.min()} "
            f"max={target_z.max()} valid=[0,{z_count - 1}]"
        )

    z0 = np.floor(target_z).astype(np.int64)
    z1 = np.minimum(z0 + 1, z_count - 1)
    alpha = (target_z - z0).astype(np.float32)
    planes = np.stack(
        [(1.0 - a) * volume[i0] + a * volume[i1]
         for i0, i1, a in zip(z0, z1, alpha)],
        axis=0,
    )
    return planes.astype(np.float32, copy=False)


def save_reference_surfaces_once(frame_idx, ref_mem_volume_xyz,
                                 ref_sparse_volume_zyx):
    """Save the complete reference images at fixed_target_z once.

    These are the actual reference-space target planes used by every moving
    frame, not a second forward projection of frame-0 moving coordinates.
    """
    global _ref_surface_saved
    if _ref_surface_saved or frame_idx != 0:
        return

    _ref_surface_saved = True
    for label, ref_volume, ref_order, out_dir in (
        ("mem", ref_mem_volume_xyz, "xyz", DIRS["ref_surface_mem"]),
        ("sparseCell", ref_sparse_volume_zyx, "zyx", DIRS["ref_surface_sparseCell"]),
    ):
        surface = sample_reference_at_fixed_z(
            ref_volume,
            fixed_target_z,
            ref_volume_order=ref_order,
        )
        surface = np.asarray(surface, dtype=np.float32)
        fh.save_single_channel_ome_tiff(
            surface,
            str(out_dir),
            frame_idx=0,
        ...
        print(
            f"  ref_surface {label}: shape={surface.shape} "
            f"finite={float(np.mean(np.isfinite(surface))):.4f}"
        )
        del surface
...
for i in range(0, T):
    ...
    # Frame-0 verification: the saved phase_new must be exactly the phase used
    # to create the coordinate array passed to projection.
    if i == 0:
        phase_reloaded = np.load(str(phase_new_path), allow_pickle=False)
        phase_exact = bool(np.array_equal(phase_reloaded, phase_new))
        phase_reload_for_proj = fh.upsample_phase_xy_for_supersurface(
            phase_reloaded, upsample_factor=2
        )
        phase_for_proj_exact = bool(
            np.array_equal(phase_reload_for_proj, phase_for_proj)
        )
        phase_verification = {
            "frame": 0,
            "saved_path": str(phase_new_path),
            "phase_new_shape": list(phase_new.shape),
            "phase_for_proj_shape": list(phase_for_proj.shape),
            "phase_new_exact_match": phase_exact,
            "phase_for_proj_exact_match_from_saved_phase": phase_for_proj_exact,
            "phase_new_max_abs_diff": float(
                np.max(np.abs(phase_reloaded - phase_new))
            ),
            "phase_for_proj_max_abs_diff": float(
                np.max(np.abs(phase_reload_for_proj - phase_for_proj))
            ),
        }
        print(
            "  phase verification: "
            f"phase_new_exact={phase_exact}, "
            f"phase_for_proj_exact={phase_for_proj_exact}"
        )
        del phase_reloaded, phase_reload_for_proj

    # Reference surfaces are the complete reference-volume planes at the same
    # fixed target z positions used by all moving-frame projections.
    save_reference_surfaces_once(
        i,
        ref_mem_adj,
        ref_sparse_raw,
    )
...
df_mem.to_csv(str(DIAGNOSTICS_DIR / "errors_membrane.csv"), index=False)
df_sparse.to_csv(str(DIAGNOSTICS_DIR / "errors_sparse.csv"), index=False)
df_holes.to_csv(str(DIAGNOSTICS_DIR / "hole_summary.csv"), index=False)
with open(DIAGNOSTICS_DIR / "phase_verification.json", "w", encoding="utf-8") as f:
    json.dump(phase_verification, f, indent=2)
